codeunit 50032 "Attribute Adjustment Mgt"
{
    ...
    procedure GetAmountFromAttributeFormula(Formula: Code[100]; EmployeeCode: Code[20]): Decimal
    begin
        if Formula = '' then
            exit(0);
        exit(EvaluateAmount(Formula, EmployeeCode));
    end;

    local procedure EvaluateAmount(Expression: Code[100]; EmployeeCode: Code[20]): Decimal
    var
        OperatorStack: array[100] of Code[20];
        NumberStack: array[100] of Decimal;
        DecNumber: Decimal;
        ...
        operat: Code[20];
    begin
        ResolveColumn(Expression, EmployeeCode);
        Expression := DelChr(Expression, '=', ',');
        ExNo := StrLen(Expression);
        if ExNo = 0 then
            exit(0);
        ...
        repeat
            ...
            if Expression[Counter] = '(' then begin
                ...
                if OsNo <> 0 then
                    while (OsNo <> 0) and (CheckPrecedenceline(OperatorStack[OsNo]) >= CheckPrecedenceline(Format(Expression[Counter]))) do begin
                        Num2 := NumberStack[NsNo];
                        NumberStack[NsNo] := 0;
                        NsNo -= 1;
                        Num1 := NumberStack[NsNo];
    ...
    local procedure CheckPrecedenceline(Opt: Code[20]): Integer
    begin
        if (Opt = '*') or (Opt = '/') then
            exit(2);
        if (Opt = '+') or (Opt = '-') then
    ...
    local procedure ResolveColumn(var Expression: Code[100]; EmployeeCode: Code[20])
    var
        StrPosition: Integer;
        StrLength: Integer;
        PayrollAttributes: Record "Payroll Attributes";
