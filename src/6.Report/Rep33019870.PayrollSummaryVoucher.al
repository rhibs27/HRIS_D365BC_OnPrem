report 33019870 "Payroll Summary Voucher"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/6.Report/Rep33019870.PayrollSummaryVoucher.rdl';
        ...
        dataitem("Posted Payroll Header"; "Posted Payroll Header")
        {
            ...
            begin
                ...
                if PayrollAttributes.FindFirst then
                    repeat
                        GLAccount.Reset;
                        GLAccount.SetFilter("No.", PayrollAttributes."G/L Account No.");
                        GLAccount.SetFilter("Document No. Filter", "Posted Payroll Header".GetFilter("No."));
                        if GLAccount.FindFirst then
                            GLAccount.CalcFields("Debit Amount", "Credit Amount");

                        if (GLAccount."Debit Amount" <> 0) or (GLAccount."Credit Amount" <> 0) then begin
                            ...
                            TempPayrollAttributes."Unit Cost" := GLAccount."Debit Amount";
                            TempPayrollAttributes."Budgeted Amount" := GLAccount."Credit Amount";
                            TempPayrollAttributes.Insert;
                            TotalDebitAmt += GLAccount."Debit Amount";
                        end;
